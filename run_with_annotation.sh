...
cd python && uv run python tests/annotate.py &
ANNOTATION_PID=$!
...
if [ "$RTSP_URL" == "0" ]; then
    cd python && uv run python main.py --mode intelligent --url 0 &
else
    cd python && uv run python main.py --mode intelligent --url "$RTSP_URL" &
fi
CAPTURE_PID=$!
